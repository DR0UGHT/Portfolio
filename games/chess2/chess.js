...
window.onload = function () {
    ...
    // let fen = 'K7/8/2k5/8/8/8/8/1q6 b - - 0 1'
    // LoadFen(fen);
...
async function ComputerMove(){
    ...
    while(inGame){
        ...
        var isGameOver = await CheckForGameOver();

        if(isGameOver != 'Game continues'){
            GameOver(isGameOver);
            break;
        }
        ...
        await MovePieces(from, to);
        currentTurn = 'w';
        document.getElementById('chessTurn').innerHTML = 'White\'s turn';
...
function GameOver(reason){
    ...
    document.getElementsByClassName('gameOver')[0].children[0].children[1].innerHTML = reason;
}
...
function FENInput(){
    ...
    let f = s=>[...s].map(c=>++n%9?+c?n+=--c:a[i='pP/KkQqRrBbNn'.search(c),i&=i>4&a[i]>(i>6)||i]=-~a[i]:x+=c=='/',a=[x=n=0])&&!([p,P,s,k,K]=a,n-71|x-7|s|k*K-1|p>8|P>8);
    ...
    if(!(fenParts[2] === '-'  || !fenParts[2].includes('K') || !fenParts[2].includes('Q') || !fenParts[2].includes('k') || !fenParts[2].includes('q'))){
        alert('Invalid FEN');
        return;
    }

    if(fenParts[3] !== '-' && (fenParts[3].charAt(0) < 'a' || fenParts[3].charAt(0) > 'h' || fenParts[3].charAt(1) < '1' || fenParts[3].charAt(1) > '8')){
        alert('Invalid FEN');
        return;
    }

    if(isNaN(fenParts[4]) || fenParts[4] < 0){
        alert('Invalid FEN');
        return;
    }

    if(isNaN(fenParts[5]) || fenParts[5] < 0){
        alert('Invalid FEN');
        return;
    }
...
async function MovePieces(from, to){
    fullMoveNumber++;
    ...
    if(toCell.children.length > 0){
        if(currentTurn === 'w'){
            document.getElementsByClassName('removedBlackPieces')[0].appendChild(toCell.children[toCell.children.length-1]);
            document.getElementsByClassName('removedBlackPieces')[0].children[document.getElementsByClassName('removedBlackPieces')[0].children.length-1].className = 'removedPiece';
        }else{
            document.getElementsByClassName('removedWhitePieces')[0].appendChild(toCell.children[toCell.children.length-1]);
            document.getElementsByClassName('removedWhitePieces')[0].children[document.getElementsByClassName('removedWhitePieces')[0].children.length-1].className = 'removedPiece';
        }
        halfMoveClock = 0;
    }
    ...
    if(currentTurn === 'w'){
        currentTurn = 'b';
        document.getElementById('chessTurn').innerHTML = 'Black\'s turn';
...
function OnlyKingsLeft(col = 'none'){
    let anyOtherPiece = false;
    for(let i = 0; i < 8; i++){
        for(let j = 0; j < 8; j++){
            if(col === 'none'){
                if(chessGrid[i][j] !== 'K' && chessGrid[i][j] !== 'k' && chessGrid[i][j] !== '--'){
                    anyOtherPiece = true;
                }
            }else if(col === 'w'){
                if(chessGrid[i][j] !== 'K' && chessGrid[i][j] !== '--'){
                    anyOtherPiece = true;
                }
            }else if(col === 'b'){
                if(chessGrid[i][j] !== 'k' && chessGrid[i][j] !== '--'){
    ...
    return !anyOtherPiece;
}

function GetSidePieceCount(color){
    let rookCount = 0;
    let bishopCount = 0;
    let knightCount = 0;
    let queenCount = 0;
    ...
    for(let i = 0; i < 8; i++){
        for(let j = 0; j < 8; j++){
            if(color === 'w'){
                if(chessGrid[i][j] === 'R'){
                    rookCount++;
                }else if(chessGrid[i][j] === 'B'){
                    bishopCount++;
                }else if(chessGrid[i][j] === 'N'){
                    knightCount++;
                }else if(chessGrid[i][j] === 'Q'){
                    queenCount++;
                }else if(chessGrid[i][j] === 'P'){
                    pawnCount++;
                }
            }else{
                if(chessGrid[i][j] === 'r'){
    ...
    return [rookCount, bishopCount, knightCount, queenCount, pawnCount];
}

function OnlyKingAndMinorPiece(color){
    let pieceCount = GetSidePieceCount(color);
    console.log(pieceCount);
    if(pieceCount[0] === 0 && pieceCount[3] === 0 && pieceCount[4] === 0){
        if(pieceCount[1] === 0 && pieceCount[2] === 1){
            return true;
        }else if(pieceCount[1] === 1 && pieceCount[2] === 0){
            return true;
        }else if(pieceCount[1] === 0 && pieceCount[2] === 2){
            return true;
        }else{
            return false;
        }
...
async function CheckForGameOver(){
    ...
    }else if(OnlyKingAndMinorPiece(currentTurn !== 'w' ? 'w' : 'b')){
        return 'Draw - Insufficient material';
    }
    ...
    console.log('fen : ' + chessGridToFen(chessGrid));
    ...
    if(ret === '(none)'){
        console.log("ARE WE IN CHECK" + IsInCheck('black'));
        if(IsInCheck('white') && currentTurn === 'w'){
            return 'Black wins - checkmate';
        }else if(IsInCheck('black') && currentTurn === 'b'){
...
function CheckForConditions(from, to){
    ...
    if(piece == 'P'){
        ...
        if(fromRow === 2 && toRow === 4){
            ...
            chessGrid[fromRow-1][fromCol-1] = 'Q';
            document.getElementById(from).children[0].src = '../images/whiteQueen.png';
            document.getElementById(from).children[0].setAttribute('onclick', 'ClickAPiece(\'Q\', \''+from+'\')');
            enPassant = '-';
        }else if(fromCol !== toCol && enPassant === rowColToAlgebraic(toRow, toCol)){
            chessGrid[toRow-2][toCol-1] = '--';
            let skippedCell = document.getElementById(rowColToAlgebraic(toRow-1, toCol));
            
            document.getElementsByClassName('removedBlackPieces')[0].appendChild(skippedCell.children[0]);
            document.getElementsByClassName('removedBlackPieces')[0].children[document.getElementsByClassName('removedBlackPieces')[0].children.length-1].className = 'removedPiece';
        ...
        if(fromRow === 7 && toRow === 5){
            ...
            chessGrid[fromRow-1][fromCol-1] = 'q';
            document.getElementById(from).children[0].src = '../images/blackQueen.png';
            document.getElementById(from).children[0].setAttribute('onclick', 'ClickAPiece(\'q\', \''+from+'\')');
            enPassant = '-';
        }else if(fromCol !== toCol && enPassant === rowColToAlgebraic(toRow, toCol)){
            chessGrid[toRow][toCol-1] = '--';
            let skippedCell = document.getElementById(rowColToAlgebraic(toRow+1, toCol));

            document.getElementsByClassName('removedWhitePieces')[0].appendChild(skippedCell.children[0]);
            document.getElementsByClassName('removedWhitePieces')[0].children[document.getElementsByClassName('removedWhitePieces')[0].children.length-1].className = 'removedPiece';
...
function ClieckedPiece(pieceType, row, col){
    ...
    switch(pieceType){
        case 'P':
            ...
            if(enPassant !== '-'){
                let enPassantRow = algebraicToRowCol(enPassant)[0];
                let enPassantCol = algebraicToRowCol(enPassant)[1];
                if(row === enPassantRow - 1 && (col === enPassantCol - 1 || col === enPassantCol + 1)){
                    placesToMarker.push([(enPassantRow-1)+(currentTurn === 'w' ? 1 : -1), enPassantCol]);
                }
            }
            break;
        case 'R':
            ...
            GetAllRookMoves(row, col).forEach(move => {
                if(!PositionInArray(allWhitePositions, move)){
                    if(!isInCheck || DoesMoveStopCheck(row, col, move[0], move[1])){
                        placesToMarker.push(move);
                    }
        ...
        case 'N':
            ...
            GetAllKnightMoves(row, col).forEach(move => {
                if(!PositionInArray(allWhitePositions, move)){
                    if(!isInCheck || DoesMoveStopCheck(row, col, move[0], move[1])){
                        placesToMarker.push(move);
                    }
        ...
        case 'B':
            ...
            GetAllBishopMoves(row, col).forEach(move => {
                if(!PositionInArray(allWhitePositions, move)){
                    if(!isInCheck || DoesMoveStopCheck(row, col, move[0], move[1])){
                        placesToMarker.push(move);
                    }
        ...
        case 'Q':
            ...
            GetAllQueenMoves(row, col).forEach(move => {
                if(!PositionInArray(allWhitePositions, move)){
                    if(!isInCheck || DoesMoveStopCheck(row, col, move[0], move[1])){
                        placesToMarker.push(move);
                    }
        ...
        case 'K':
            ...
            GetAllKingMoves(row, col).forEach(move => {
                if(!PositionInArray(allWhitePositions, move)){
                    if(!isInCheck || DoesMoveStopCheck(row, col, move[0], move[1])){
                        if(!PositionInArray(attackedSquaresBlack, move)){
                            placesToMarker.push(move);
                        }
                    }
                }
            ...
            break;
        case 'p':
            var allBlackPositions = GetAllPiecesPositions('black');
        ...
        case 'k':
            ...
            GetAllKingMoves(row, col).forEach(move => {
                if(!PositionInArray(attackedSquaresWhite, move)){
                    if(!isInCheck || DoesMoveStopCheck(row, col, move[0], move[1])){
                        if(!PositionInArray(allBlackPositions, move)){
                            placesToMarker.push(move);
                        }
    ...
    for(var i = 1; i < 9; i++){ 
        for(var j = 1; j < 9; j++){
            if(GetPieceColorAtPosition(i, j) === pieceColor){
                ...
                switch(pieceType){
                    case 'P':
                        GetAllPawnMoves(row, col, 'white', true, false).forEach((newPos) => { placesToMarker.push(newPos); });
                        break;
                    case 'R':
                    ...
                    case 'K':
                        ...
                        break;
                    case 'p':
                        GetAllPawnMoves(row, col, 'black', true, false).forEach((newPos) => { placesToMarker.push(newPos); });
                        break;
                    case 'r':
                        GetAllRookMoves(row, col).forEach((newPos) => { placesToMarker.push(newPos); });
                        break;
